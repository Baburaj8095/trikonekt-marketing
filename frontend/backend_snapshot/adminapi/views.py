...
class AdminMatrix5Tree(APIView):
    ...
    def get(self, request):
        ...
        if pool not in ("FIVE_150", "THREE_150", "THREE_50"):
            pool = "FIVE_150"
        # Cap max_depth to configured levels per pool to avoid heavy renders
        try:
            cfg = CommissionConfig.get_solo()
            if pool == "FIVE_150":
                configured_levels = int(getattr(cfg, "five_matrix_levels", 6) or 6)
            else:
                configured_levels = int(getattr(cfg, "three_matrix_levels", 15) or 15)
            if configured_levels > 0:
                max_depth = max(1, min(int(max_depth), int(configured_levels)))
        except Exception:
            pass
        ...
        # Short cache to avoid repeated recomputation during admin navigation
        try:
            cache_key = f"adm_tree5:{pool}:{source}:{user.id}:{int(max_depth)}:{1 if sponsor_wide else 0}"
        except Exception:
            cache_key = None
        if cache_key:
        ...
        def build_autopool(u, level: int, pool_type="FIVE_150", visited=None, ap_pos=None, anchor_parent_ids=None):
            # Batched BFS over AutoPoolAccount graph, limited to fanout per parent and NODE_CAP nodes total.
            ...
            node = {
                ...
                "full_name": u.full_name,
                "level": level,
                "matrix_position": ap_pos,
                "ap_position": ap_pos,
                "depth": getattr(u, "depth", 0),
                "children": [],
            }
            # shallow return when depth budget is 1
            if max_depth <= 1 or NODE_CAP <= 1:
                node_budget["count"] = min(node_budget["count"] + 1, NODE_CAP)
                return node
            try:
            ...
            # Establish initial anchor accounts for this user
            parent_acc_ids = []
            try:
                if anchor_parent_ids:
                    parent_acc_ids = [int(x) for x in (anchor_parent_ids or []) if x]
                elif sentinel and getattr(sentinel, "owner_id", None) == getattr(u, "id", None) and level == 1:
                    ...
                    primary_id = (
                        ...
                        .first()
                    ) if AutoPoolAccount else None
                    parent_acc_ids = [int(primary_id)] if primary_id else []
            except Exception:
                parent_acc_ids = []
            ...
            # BFS frontier of account ids mapped to their owning user nodes
            parent_to_node = {int(pid): node for pid in parent_acc_ids}
            current_anchors = [int(pid) for pid in parent_acc_ids]
            current_level = level
            total_nodes = 1  # includes root
            node_budget["count"] = min(node_budget["count"] + 1, NODE_CAP)

            while current_anchors and current_level < max_depth and total_nodes < NODE_CAP:
                try:
                    qs = (
                        ...
                        .filter(
                            pool_type=pool_type,
                            status="ACTIVE",
                            parent_account_id__in=current_anchors,
                            position__lte=fanout,
                        )
                        .exclude(Q(source_type__in=["SEED", "FORCE_SEED", "RESEAT", "SENTINEL"]) & ~Q(owner_id=u.id))
                        .order_by("parent_account_id", "position", "id")
                    )
                except Exception:
                    qs = []

                # Track per-parent fanout and per-parent owner de-duplication
                per_parent_counts = {}
                used_owners_map = {}
                next_anchors = []
                next_parent_to_node = {}
                ...
                for a in qs:
                    ...
                    parent_node = parent_to_node.get(int(pid)) if pid is not None else None
                    if not parent_node:
                        continue
                    c = per_parent_counts.get(pid, 0)
                    if c >= fanout or total_nodes >= NODE_CAP:
                        continue
                    ou = getattr(a, "owner", None)
                    oid = getattr(ou, "id", None)
                    if not oid:
                        continue
                    if str(getattr(ou, "category", "") or "").lower() != "consumer":
                        continue
                    used = used_owners_map.setdefault(pid, set())
                    if oid in used:
                        continue
                    # Create child user node
                    child = {
                        "id": oid,
                        "username": getattr(ou, "username", None),
                        "full_name": getattr(ou, "full_name", None),
                        "level": current_level + 1,
                        "matrix_position": getattr(a, "position", None),
                        "ap_position": getattr(a, "position", None),
                        "depth": getattr(ou, "depth", 0),
                        "children": [],
                    }
                    parent_node["children"].append(child)
                    used.add(oid)
                    per_parent_counts[pid] = c + 1
                    total_nodes += 1
                    node_budget["count"] = min(node_budget["count"] + 1, NODE_CAP)
                    # Next frontier: expand from this exact child account id
                    next_anchors.append(int(getattr(a, "id", 0)))
                    next_parent_to_node[int(getattr(a, "id", 0))] = child
                    if total_nodes >= NODE_CAP:
                        break

                current_anchors = next_anchors
                parent_to_node = next_parent_to_node
                current_level += 1
        ...
        # Decide source
        if source == "matrix":
            # Strict matrix source: only show active autopool graph for the requested pool.
            # Do NOT fall back to CustomUser.parent-based graph, since that may include inactive/unopened accounts.
            tree = build_autopool(user, 1, pool_type=pool)
            try:
                if cache_key:
                    cache.set(cache_key, tree, timeout=10)
            except Exception:
                pass
            return Response(tree, status=200)
        if source == "sponsor":
        ...
        if not mx_tree or not isinstance(mx_tree.get("children", []), list) or len(mx_tree["children"]) == 0:
            ...
            if ap_tree and isinstance(ap_tree.get("children", []), list) and len(ap_tree["children"]) > 0:
                try:
                    if cache_key:
                        cache.set(cache_key, ap_tree, timeout=10)
                except Exception:
                    pass
            ...
            try:
                if cache_key:
                    cache.set(cache_key, sp_tree, timeout=10)
            except Exception:
                pass
            return Response(sp_tree, status=200)
        try:
            if cache_key:
                cache.set(cache_key, mx_tree, timeout=10)
        except Exception:
            pass
