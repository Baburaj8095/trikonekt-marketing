...
class AdminPromoPurchaseApproveView(APIView):
    ...
    def post(self, request, pk: int):
        ...
        with transaction.atomic():
            ...
            # Inline matrix opening/payouts for PRIME approvals (150/750) to ensure matrix nodes exist immediately
            distributed_inline = False
            try:
                from business.services.activation import activate_150_active
                if is_prime_150:
                    # Directly open 5/3 matrix accounts and distribute for PRIME 150
                    activate_150_active(obj.user, {"type": "PROMO_PURCHASE_APPROVAL", "id": obj.id})
                    distributed_inline = True
                elif is_prime_750:
                    # For PRIME 750, open 5/3 matrix accounts and distribute inline with 750-aware logic
                    try:
                        activate_150_active(obj.user, {"type": "PRIME_750", "id": obj.id})
                        distributed_inline = True
                    except Exception:
                        pass
            except Exception:
                # best-effort; failure here should not block approval
                pass
            # Stamp audit to prevent background duplicate enqueue if inline handled
            try:
                if distributed_inline:
                    from coupons.models import AuditTrail
                    AuditTrail.objects.create(
                        action="promo_purchase_distributed",
                        actor=obj.approved_by,
                        notes="Inline prime payouts/matrix during promo approval",
