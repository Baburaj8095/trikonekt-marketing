...
function useDemoData() {
  ...
  const products = useMemo(() => {
    ...
    const base = [
      ...
      { name: "Casual Cotton Tâ€‘Shirt", cat: "fashion", price: 499, qty: 80 },
      { name: "Herbal Facewash", cat: "health-beauty", price: 199, qty: 60 },
      { name: "Grocery Delivery Combo", cat: "local-store", price: 999, qty: 30 },
    ];
    return base.map((p, i) => ({
      ...p,
      description: `${p.name} â€” Demo product seeded at ${new Date(now).toLocaleString()}.`,
      discount: i % 3 === 0 ? 10 : 0,
    ...
    <div>
      ...
      <Section
        ...
        action={
          ...
          >
            {busy ? "Workingâ€¦" : "Create 10 categories + 10 products"}
          </button>
        }
      >
            ...
            <ul style={{ margin: 0, paddingLeft: 16 }}>
              {products.map((p) => (
                <li key={p.name} style={{ fontSize: 13, color: "#334155" }}>
                  {p.name} <span style={{ color: "#64748b" }}>[{p.cat}]</span> â€” â‚¹{p.price} ({p.qty} qty)
                </li>
              ))}
            </ul>
          </div>
      ...
      <Section title="Logs">
        ...
        {done ? (
          <div style={{ marginTop: 8, color: "#16a34a", fontWeight: 700 }}>
            Completed. Check Admin â†’ Eâ€‘commerce Categories and Admin â†’ Products, and the user page /trikonekt-products.
          </div>
        ) : null}
      </Section>
    </div>
