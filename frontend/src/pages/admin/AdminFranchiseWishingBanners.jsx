...
export default function AdminFranchiseWishingBanners() {
  ...
  const fetchRows = async () => {
    ...
    setErr("");
    try {
      const res = await API.get("/business/admin/franchise/wishing-banners/", { params: { page_size: 500 } });
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (form.id) {
        await API.patch(`/business/admin/franchise/wishing-banners/${form.id}/`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        setNotice("Banner updated.");
      } else {
        await API.post("/business/admin/franchise/wishing-banners/", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const onDelete = async (row) => {
    ...
    setNotice("");
    try {
      await API.delete(`/business/admin/franchise/wishing-banners/${row.id}/`);
      setNotice("Deleted.");
      await fetchRows();
    } catch (e) {
        ...
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
          <Box>
            <Typography variant="h5" sx={{ fontWeight: 900 }}>
              Franchise • Wishing Banners
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Admin uploads banner images; franchise dashboard displays active banners.
            </Typography>
          </Box>
