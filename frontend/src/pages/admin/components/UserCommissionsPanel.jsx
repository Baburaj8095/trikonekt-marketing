  ...
  const columns = useMemo(() => {
    return [
      ...
      { field: "coupon_code", headerName: "Coupon Code", width: 160 },
      {
        ...
        minWidth: 120,
        width: 140,
        renderCell: (p) => (p?.row?.coupon_code_value != null ? `â‚¹${Number(p.row.coupon_code_value).toFixed(2)}` : "â€”"),
      },
    ];
  }, []);
      ...
      >
        ...
        >
          <div style={{ fontWeight: 900, color: "#0f172a" }}>
            Commissions â€” {username ? `${username}` : ""}{userId ? ` (#${userId})` : ""}{role ? ` [${role}]` : ""}
          </div>
          <button
            onClick={onClose}
            style={{
        ...
        <div style={{ padding: 12, borderBottom: "1px solid #e2e8f0" }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <div style={{ padding: 10, borderRadius: 8, background: "#f8fafc", border: "1px solid #e2e8f0", minWidth: 160 }}>
              ...
              <div style={{ fontWeight: 900, color: "#0f172a" }}>{summary.counts.earned} â€¢ â‚¹{summary.totals.earned}</div>
            </div>
            <div style={{ padding: 10, borderRadius: 8, background: "#f8fafc", border: "1px solid #e2e8f0", minWidth: 160 }}>
              <div style={{ fontSize: 12, color: "#64748b", fontWeight: 700 }}>Paid</div>
              <div style={{ fontWeight: 900, color: "#0f172a" }}>{summary.counts.paid} â€¢ â‚¹{summary.totals.paid}</div>
            </div>
            <div style={{ padding: 10, borderRadius: 8, background: "#f8fafc", border: "1px solid #e2e8f0", minWidth: 160 }}>
              <div style={{ fontSize: 12, color: "#64748b", fontWeight: 700 }}>Reversed</div>
              <div style={{ fontWeight: 900, color: "#0f172a" }}>{summary.counts.reversed} â€¢ â‚¹{summary.totals.reversed}</div>
            </div>
            <div style={{ padding: 10, borderRadius: 8, background: "#f8fafc", border: "1px solid #e2e8f0", minWidth: 160 }}>
              <div style={{ fontSize: 12, color: "#64748b", fontWeight: 700 }}>Total</div>
              <div style={{ fontWeight: 900, color: "#0f172a" }}>{summary.counts.total} â€¢ â‚¹{summary.totals.total}</div>
            </div>
