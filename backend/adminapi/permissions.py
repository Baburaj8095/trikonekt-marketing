...
def has_admin_module_access(user, module_key: str) -> bool:
    ...
    try:
        ...
        wanted = {mk, f"screen_{mk}", f"module_{mk}"}
...
def get_effective_permissions(user) -> set[str]:
    ...
    try:
        ...
        if not role:
            return set()
        if getattr(role, "is_super", False):
            return {"*"}
        # Local import to avoid circulars at import time
        from .models import RolePermission
        rows = RolePermission.objects.select_related("permission").filter(role_id=getattr(role, "id", None))
        codes = set()
        for r in rows:
            try:
